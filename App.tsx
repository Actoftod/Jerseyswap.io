            ...
            <AnimatePresence mode="wait">
              ...
              {authMode === '2fa' && (
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="space-y-6">
                  ...
                  <input aria-label="2FA Verification Code" value={twoFACode} onChange={e => setTwoFACode(e.target.value.slice(0,4))} type="text" className="w-full bg-white/5 border border-white/10 rounded-2xl py-6 text-center text-4xl font-oswald italic font-black text-[#ccff00]" placeholder="1234" />
        ...
        <div className="w-full flex flex-col items-center">
          <header className="fixed top-0 inset-x-0 z-[100] bg-black/60 backdrop-blur-2xl border-b border-white/5 px-6 h-14 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button aria-label="Social Feed" title="Social Feed" onClick={() => setStep('social-feed')} className={`p-2 rounded-lg ${step === 'social-feed' ? 'text-[#ccff00]' : 'text-zinc-500'}`}><Globe className="w-5 h-5" /></button>
              <button aria-label="AI Lab" title="AI Lab" onClick={() => setShowAILab(!showAILab)} className={`p-2 rounded-lg ${showAILab ? 'text-[#ccff00]' : 'text-zinc-500'}`}><Cpu className="w-5 h-5" /></button>
              <button aria-label="My Profile" title="My Profile" onClick={() => { setViewingProfile(null); setStep('profile'); }} className={`p-2 rounded-lg ${step === 'profile' && !viewingProfile ? 'text-[#ccff00]' : 'text-zinc-500'}`}><LayoutGrid className="w-5 h-5" /></button>
              <button aria-label="Logout" title="Logout" onClick={logout} className="p-2 text-zinc-700 hover:text-red-500"><LogOut className="w-5 h-5" /></button>
            </div>
          </header>
