      ...
      {(step as string === 'auth' || !activeProfile) && step !== 'onboarding' ? (
              ...
              {authMode === 'signin' && (
                 <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                   <div className="space-y-4">
                     <input placeholder="EMAIL_ADDRESS" aria-label="Email Address" value={authEmail} onChange={e => setAuthEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-6 font-oswald italic text-white outline-none" />
                     <input type="password" placeholder="PASSWORD" aria-label="Password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-6 font-oswald italic text-white outline-none" />
                   </div>
              ...
              {authMode === 'signup' && (
                 <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                   <div className="space-y-3">
                     <input placeholder="FULL_NAME" aria-label="Full Name" value={authName} onChange={e => setAuthName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 font-oswald italic text-white" />
                     <input placeholder="EMAIL" aria-label="Email Address" value={authEmail} onChange={e => setAuthEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 font-oswald italic text-white" />
                     <input placeholder="@HANDLE" aria-label="Handle" value={authHandle} onChange={e => setAuthHandle(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 font-oswald italic text-white" />
                     <input type="password" placeholder="PASSWORD" aria-label="Password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 font-oswald italic text-white" />
                   </div>
        ...
        <div className="w-full flex flex-col items-center">
          <header className="fixed top-0 inset-x-0 z-[100] bg-black/60 backdrop-blur-2xl border-b border-white/5 px-6 h-14 flex items-center justify-between">
            <button onClick={reset} aria-label="Home" title="Home" className="flex items-center gap-2 cursor-pointer bg-transparent border-none p-0 hover:opacity-80 transition-opacity">
            ...
            <div className="flex gap-2">
              <button onClick={() => setStep('social-feed')} aria-label="Social Feed" title="Social Feed" className={`p-2 rounded-lg ${step === 'social-feed' ? 'text-[#ccff00]' : 'text-zinc-500'}`}><Globe className="w-5 h-5" /></button>
              <button onClick={() => setShowAILab(!showAILab)} aria-label="AI Lab" title="AI Lab" className={`p-2 rounded-lg ${showAILab ? 'text-[#ccff00]' : 'text-zinc-500'}`}><Cpu className="w-5 h-5" /></button>
              <button onClick={() => { setViewingProfile(null); setStep('profile'); }} aria-label="My Profile" title="My Profile" className={`p-2 rounded-lg ${step === 'profile' && !viewingProfile ? 'text-[#ccff00]' : 'text-zinc-500'}`}><LayoutGrid className="w-5 h-5" /></button>
